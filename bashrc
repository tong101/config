...
gitpush()
{
  GIT_REPO=$(git config --get remote.origin.url | sed 's#.*/##')
  if [ -n "$GIT_REPO" ]; then
    GIT_BRANCH=$(git rev-parse --abbrev-ref HEAD)
    GIT_CMD="git push review HEAD:refs/for/$GIT_BRANCH"
    echo $GIT_CMD
    $GIT_CMD
  else
    echo "Not in a git working repo"
  fi
}
...
alias kh='rm -f ~/.ssh/known_hosts; touch ~/.ssh/known_hosts'
